...
then
	...
	[ -d bin ] && rm -rf bin
	wget https://github.com/containerd/containerd/releases/download/v2.0.1/containerd-2.0.1-linux-${PLATFORM}.tar.gz >/dev/null 2>&1
	tar xvf containerd-2.0.1-linux-${PLATFORM}.tar.gz
	sudo mv bin/* /usr/bin/
